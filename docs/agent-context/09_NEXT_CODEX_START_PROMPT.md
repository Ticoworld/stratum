...
Phases 1, 2, 3, 4, 5, 6, and 7 are already complete. Do not redo them. Do not remove or bypass the completed foundation, provider freeze path, report-run worker foundation, structured analysis layer, publication layer, artifact layer, or the Phase 7 product-facing UI cutover.
...
The next approved phase is Phase 8. Do not drift beyond Phase 8. Do not add later-phase scope unless the current approved phase explicitly requires it.
