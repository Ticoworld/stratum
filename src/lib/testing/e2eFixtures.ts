        ...
        : [
            {
              ...
              errorMessages: [],
              usedForBrief: true,
              note: `Returned ${jobs.length} observed open role${jobs.length === 1 ? "" : "s"}. Used for this brief.`,
            },
          ],
...
export async function seedE2eFixtures(runKey: string): Promise<SeededE2eFixtures> {
  ...
  const phase9OlderBriefId = await insertSavedObservedBrief({
    ...
    watchlistReadExplanation: "Read confidence is low because only one observed role supports this brief.",
